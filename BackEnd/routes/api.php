...
use App\Http\Middleware\WorkingHoursMiddleware;

// ═══════════════════════════════════════════════════════════════════════════
// ALWAYS ACCESSIBLE (Tidak dibatasi jam kerja)
// ═══════════════════════════════════════════════════════════════════════════
...
// --- Health Check & Operating Status ---
Route::get('/health', fn() => response()->json(['status' => 'ok']));

Route::get('/operating-status', function () {
    $isOpen = WorkingHoursMiddleware::isOpen();
...
Route::middleware('working-hours')->group(function () {
    ...
    // Global delete with Admin verification
    Route::delete('/queues', function (Request $request) {
        if (!$request->user() || !in_array($request->user()->role, ['Admin Dev', 'AdminDev', 'admin'])) {
            return response()->json(['message' => 'Unauthorized. Hanya Admin yang diizinkan.'], 403);
        }
